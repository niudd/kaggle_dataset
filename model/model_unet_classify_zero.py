...
#         pred = self.sigmoid(logit.cpu().detach())>0.5
#         truth = truth.cpu().detach()
#         tn, fp, fn, tp = confusion_matrix(truth, pred).ravel()
#         #auc = roc_auc_score(truth, pred)
#         #return auc, tn, fp, fn, tp
#         return round(fn/(tn+fn), 5), tn, fp, fn, tp, round(roc_auc_score(truth, pred), 5) #use false negative rate to early stop
    
#     def metric(self, logit, truth):
#         pred = self.sigmoid(logit.cpu().detach().numpy())
#         truth = truth.cpu().detach().numpy()
#         ##
#         THRESHOLD_candidate = np.arange(0.4, 0.6, 0.01)
#         N = len(THRESHOLD_candidate)
#         best_threshold = None
#         best_score = 9999
...
#             else:
#                 fn_rate = round(_fn/(_tn+_fn), 5)
#             #predict positive proportion should be ~12%
#             _pos_percent = (_tp+_fp)/(_tp+_fp+_tn+_fn)
#             if 0.45<_pos_percent<0.55:
#                 if fn_rate < best_score:
#                     best_threshold = THRESHOLD
#                     best_score = fn_rate
#                     tn, fp, fn, tp, auc, pos_percent = _tn, _fp, _fn, _tp, _auc, _pos_percent
#         return best_score, tn, fp, fn, tp, auc, pos_percent
