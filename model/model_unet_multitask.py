...
class UNetResNet34(nn.Module):
    ...
    def __init__(self, pretrained=True, debug=False, segment_off=False):
        super().__init__()
        ...
        self.logit_clf = nn.Sequential(
            #nn.BatchNorm1d(512),
            #nn.Dropout(0.5),
            nn.Linear(256, 4), #block.expansion=1, num_classes=28
            #nn.Sigmoid()
        )
    ...
    def criterion(self, logit_mask, truth_mask, logit_clf, truth_clf):
        ...
        Loss_FUNC_mask = nn.BCEWithLogitsLoss(reduction='none')
        Loss_FUNC_clf = nn.BCEWithLogitsLoss(reduction='mean')
        #bce_loss = Loss_FUNC(logit, truth)

        loss_mask = Loss_FUNC_mask(logit_mask, truth_mask)
        if truth_clf.sum()==0:
            loss_mask_nonempty = (loss_mask.mean(dim=[2,3])*truth_clf).sum()#0.0
        else:
            loss_mask_nonempty = (loss_mask.mean(dim=[2,3])*truth_clf).sum() / truth_clf.sum() #(loss_mask.mean(dim=[2,3])*truth_clf).mean() #loss for non-empty images

        loss_clf = Loss_FUNC_clf(logit_clf, truth_clf)
        loss = 0.5*loss_mask_nonempty + 0.5*loss_clf

        #loss = L.lovasz_hinge(logit, truth, ignore=None)#255
        #loss = dice_loss(logit, truth) + bce_loss
        #loss = dice_loss(logit, truth)
        return loss, loss_mask_nonempty, loss_clf
    ...
    def metric(self, logit, truth):
        ...
        _pred, pred, truth = _pred.reshape(-1, 1), pred.reshape(-1, 1), truth.reshape(-1, 1)

        res = confusion_matrix(truth, _pred).ravel()
        if len(res)==4:
            tn, fp, fn, tp = res
        else:
            if truth[0,0]==1:
                tn, fp, fn, tp = 0,0,0,truth.shape[0]
            else:
                tn, fp, fn, tp = truth.shape[0],0,0,0
        try:
            auc = round(roc_auc_score(truth, pred), 5)#GT all one class, raise error
        except:
            auc = -1
